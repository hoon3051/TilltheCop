...
func (svc AuthService) DeleteToken(accessTokenString string, RefreshTokenString string) (int64, error) {
	...
	// Delete Accesstoken
	deleted, err := initializer.Redis.Del(context.Background(), accessUUIDString).Result()
	if err != nil {
		return 0, err
	}
	...
	// Delete Refreshtoken
	deleted, err = initializer.Redis.Del(context.Background(), refreshUUIDString).Result()
	if err != nil {
		return 0, err
	}
